...
int makeIBDClassFromRunlist(const char *fname="2018C_GoodRuns_RxStatus.txt", const char* passname = "", const char* release = "Analyzed_2018C"){
  ...
  cout<<"Attempted to add "<<nadded<<" files from good runs list to Rx On TChain\n";
  cout<<"Removed "<<nmissing<<" missing files from Rx On TChain for total of "<<nadded-nmissing<<" files\n";
 nadded = chOn->GetListOfFiles()->GetEntries();
  chOn->Lookup(1);
  nadded = chOff->GetListOfFiles()->GetEntries();
  chOff->Lookup(1);
  nmissing = nadded - chOff->GetListOfFiles()->GetEntries();
  cout<<"Attempted to add "<<nadded<<" files from good runs list to Rx Off TChain\n";
  cout<<"Removed "<<nmissing<<" missing files from Rx Off TChain for total of "<<nadded-nmissing<<" files\n";
